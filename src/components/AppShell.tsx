          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <Link href="/" className="flex items-center gap-3">
              ...
              <span>
                ...
                <span className="block text-xs text-slate-500 dark:text-slate-400">Domain sales agent</span>
              </span>
            </Link>
...
export function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <section className={`rounded-md border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950 ${className}`}>{children}</section>;
}
